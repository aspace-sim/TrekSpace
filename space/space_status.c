...
{
	int x = sdb[n].slist.sdb[contact], l;
	register int i;
	...
	//Start Custom Code""
	if (level < 25) {
		snprintf(buffer, sizeof(buffer), "%s%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s %-5.5s%s\n",
			ANSI_HILITE,ANSI_YELLOW, sdb[n].slist.num[contact], ANSI_YELLOW,
			...
			unparse_speed(sdb[x].move.out), arc2, ANSI_NORMAL);
	} else if (level < 50.0) {
				if(sdb[n].iff.freq == sdb[x].iff.freq){
					snprintf(buffer, sizeof(buffer), "%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s %-5.5s %16.16s %s%-6.6s\n",
						ANSI_GREEN, sdb[n].slist.num[contact], ANSI_GREEN,
						unparse_type(x), level,
						sdb2bearing(n, x), sdb2elevation(n, x),
						unparse_range(sdb2range(n, x)), arc1,
						sdb[x].course.yaw_out, sdb[x].course.pitch_out,
						unparse_speed(sdb[x].move.out), arc2,
						(sdb[x].cloak.active ? "(cloaked)" : unparse_class(x)), ANSI_NORMAL, contact_flags(x));
					}
					else {
					snprintf(buffer, sizeof(buffer), "%s%3d%s %-4.4s %3.0f %3.0f %-3.0f %7.7s %-5.5s %3.0f %-3.0f %6.6s %-5.5s %16.16s %s%-6.6s\n",
						ANSI_RED, sdb[n].slist.num[contact], ANSI_RED,
						unparse_type(x), level,
						sdb2bearing(n, x), sdb2elevation(n, x),
						unparse_range(sdb2range(n, x)), arc1,
						sdb[x].course.yaw_out, sdb[x].course.pitch_out,
		...
		//IFF Code - This basically checks first to see if your ship is a in a specific empire, then it checks 
		//for the IFF code on the major empire object then check sdb[n].iff.freq
		// Get the Empire ATR on the ship
		/*empire = atr_get(sdb[n].object, "EMPIRE");	
		empire1 = safe_uncompress(empire->value);
		py=atoi(empire1);*/

		//If you are a federation ship
			if(sdb[n].iff.freq == sdb[x].iff.freq){
...
int do_sensor_report (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (sdb[n].slist.lev[i] > 0.5 && !sdb[x].cloak.active) {
			...
			if (type_name[sdb[x].structure.type] != "Nebula"){
		...
		if (sdb[n].slist.lev[i] > 0.25 && !sdb[x].cloak.active) {
			...
			//do_ship_notify(n, tprintf("%s",unparse_type(x)));
			//If its a nebula we show radius not displacement
			if (type_name[sdb[x].structure.type] == "Nebula"){
			strncat(buffer, format_Radius(x), sizeof(buffer) - 1);
			}
			else {
		...
		//If its a nebula we don't show anything	
		if (type_name[sdb[x].structure.type] != "Nebula"){
...
/* -----------MOdified by Bill 4-9-2002 to show life support--------------------------- */
...
int do_eng_status (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (sdb[n].engine.warp_exist || sdb[n].engine.impulse_exist) {
			...
			//Lets show the CF - Confidence Factor
				strncat(buffer,	format_CF(n),sizeof(buffer) -1);
				strncat(buffer,"\n",sizeof(buffer) -1);
		...
		if (sdb[n].main.exist || sdb[n].aux.exist || sdb[n].batt.exist || sdb[n].engine.warp_exist || sdb[n].engine.impulse_exist) {
			...
			strncat(buffer, format_s_Life_Support(n), sizeof(buffer) - 1);
		}
		strncat(buffer, format_l_end(), sizeof(buffer) - 1);
		notify(enactor, buffer);
		return 1;
...
/* ----------------------TOUCH LOCATION REPORT-------------------------------------- */

int do_touch_location (dbref enactor)
{
	static char buffer[BUFFER_LEN];
...
int do_damage_status (int type, char *name, dbref enactor)
{
	...
	if (type) {
		...
		if (x == SENSOR_FAIL) {
			...
			notify(enactor, ansi_red("That is not a valid subject."));
			do_log (LT_SPACE, enactor, x, "BUG:Subject has bad TYPE");
			return 0;
			...
			notify(enactor, ansi_red("That is not a valid subject."));
			do_log (LT_SPACE, enactor, x, "BUG:Subject has bad flags");
			return 0;
		}
	} else
		x = n;
		...
		/*REpair Life Supported - Added by bill 4-10-2002 */
		if (sdb[x].lifesupport.damage < 1.0)
			strncat(buffer, tprintf("%s%-18.18s%s %7.7s %-15.15s %10.3f\n",
			  ...
			  (1.0 - sdb[x].lifesupport.damage) * repair_mult[15] * (1.0 + (sdb[x].structure.max_structure / 10.0)) * 100.0), sizeof(buffer) - 1);			  
...
int do_scanner_report (int a, char *s, dbref enactor)
{
	...
	switch (s[0]) {
		...
		case 'd': case 'D':
			switch (s[1]) {
			case 'e': case 'E': f = 2; break; /* defense */
			case 'a': case 'A': f=10; break; /* damage */
			default: f=10; break;
			} break;
	...
	if (error_on_console(enactor)) {
		...
		switch (f) {
			...
			case 3:
				if (sdb[x].main.exist || sdb[x].aux.exist || sdb[x].batt.exist || sdb[x].engine.warp_exist || sdb[x].engine.impulse_exist) {
					...
					if (sdb[x].engine.impulse_exist) {
						...
						strncat(buffer, format_s_Life_Support(x), sizeof(buffer) - 1);
				} else {
			...
			case 7:
				/* Tweaked by bill 4-10-2002 to add Life Support */
			...
			case 9:
				...
				if (d) {
					sp = sbuf = safe_uncompress(d->value);
			...
			case 10:
				strncat(buffer, format_Damage_History(x), sizeof(buffer) - 1);
				break;								
		}
...
int do_border_report (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		snprintf(buffer, sizeof(buffer),
		  ...
		  ANSI_HILITE, ANSI_BLUE, ANSI_YELLOW, ANSI_BLUE, ANSI_NORMAL,
		  ANSI_CYAN, ANSI_WHITE, ANSI_BLUE, ANSI_WHITE);
		for (i = 0; i < MAX_EMPIRE_NAME - 1; ++i) {
			range = xyz2range(sdb[n].coords.x, sdb[n].coords.y, sdb[n].coords.z, empire_center[i][1], empire_center[i][2], empire_center[i][3]) / PARSEC;
			if (fabs(range - empire_center[i][0]) >= 100.0)
				continue;
			strncat(buffer, tprintf("%-20.20s ", empire_name[i]), sizeof(buffer) - 1);
			if (range > empire_center[i][0]) {
				strncat(buffer, tprintf("%3d %-3d %-7.7s Outside  ",
				  (int) xy2bearing((empire_center[i][1] - sdb[n].coords.x), (empire_center[i][2] - sdb[n].coords.y)),
				  (int) xyz2elevation((empire_center[i][1] - sdb[n].coords.x), (empire_center[i][2] - sdb[n].coords.y), (empire_center[i][3] - sdb[n].coords.z)),
				  unparse_range((range - empire_center[i][0]) * PARSEC)), sizeof(buffer) - 1);
			} else {
				strncat(buffer, tprintf("%3d %-3d %-7.7s Inside   ",
				  (int) xy2bearing((sdb[n].coords.x - empire_center[i][1]), (sdb[n].coords.y - empire_center[i][2])),
				  (int) xyz2elevation((sdb[n].coords.x - empire_center[i][1]), (sdb[n].coords.y - empire_center[i][2]), (sdb[n].coords.z - empire_center[i][3])),
				  unparse_range((empire_center[i][0] - range) * PARSEC)), sizeof(buffer) - 1);
			}
			strncat(buffer, tprintf("%10.3f %10.3f %10.3f\n",
			  (empire_center[i][1] - sdb[n].coords.xo) / PARSEC,
			  (empire_center[i][2] - sdb[n].coords.yo) / PARSEC,
			  (empire_center[i][3] - sdb[n].coords.zo) / PARSEC), sizeof(buffer) - 1);
		}
		strncat(buffer, format_l_line(), sizeof(buffer) - 1);
		strncat(buffer, format_Course(n), sizeof(buffer) - 1);
		strncat(buffer, format_Speed(n), sizeof(buffer) - 1);
...
int do_planet_report (const char *s, int t, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (c) {
			snprintf(buffer, sizeof(buffer), "%s%s--[%sPlanet Report%s]--------------------------------------------------------------%s\n%sPlanet               C ETA     Bearing Range   Coordinates%s\n%s-------------------- - ------- ------- ------- --------------------------------%s\n",
			  ANSI_HILITE, ANSI_BLUE, ANSI_YELLOW, ANSI_BLUE, ANSI_NORMAL, ANSI_CYAN, ANSI_WHITE, ANSI_BLUE, ANSI_WHITE);
			for (i = 0; i < max; ++i) {
				if (pln[i])
					strncat(buffer, tprintf("%s%-20.20s%s %1.1s %7.7s %3.0f %-3.0f %7.7s %10.3f %10.3f %10.3f\n",
			...
			notify(enactor, ansi_red("No planets found."));
